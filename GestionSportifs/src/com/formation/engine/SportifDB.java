...
public class SportifDB {
	...
	/*public ArrayList<ArrayList<String>> affiche(int id) throws SQLException {
		Log.write("Affiche un sportif");
		ArrayList<String> sportif = new ArrayList<String>();
		ArrayList<ArrayList<String>> request= db.selectRequest("SELECT * FROM tp1.sportifs WHERE id="+id);
		for (ArrayList<String> s : sportifs) {
			sportif=s;
		}
		return sportifs;
	...
	public ArrayList<String> affiche(int id) throws SQLException {
		Log.write("Affiche un sportif");
		ArrayList<String> sportif = new ArrayList<String>();
		ArrayList<ArrayList<String>> request= db.selectRequest("SELECT * FROM tp1.sportifs WHERE id="+id);
		for (ArrayList<String> s : request) {
			sportif=s;
		}
		return sportif;
	}
